...
public class MoveEasyTwo : MonoBehaviour
{
    ...
    void ArrangeDistanceAtStart()
	{
		...
		{
            children[0].position += new Vector3(0.10f, 0, 0);
            children[1].position += new Vector3(0.10f * -1, 0, 0);
		}
        while(distance < moveDistance)
		{
            children[0].position += new Vector3(0.10f * -1, 0, 0);
            children[1].position += new Vector3(0.10f, 0, 0);
        }
    }
}
